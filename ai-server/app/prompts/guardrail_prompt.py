...
2. **Relevance Check (is_irrelevant)**:
   ...
   - **중요**: 프레임워크가 "fullstack"인 경우, 프론트엔드(React, Vue)와 백엔드(Spring, Django) 관련 질문 모두 관련 있는 것으로 판단하세요.
   - 예시:
     ...
     * 프레임워크가 "fullstack"인데 "Spring @Autowired가 안 돼요" → is_irrelevant: false
     * 프레임워크가 "fullstack"인데 "React useEffect 의존성 문제" → is_irrelevant: false
