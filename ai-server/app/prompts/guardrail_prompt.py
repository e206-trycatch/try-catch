...
2. **Relevance Check (is_irrelevant)**:
   - **최우선 규칙**: 프레임워크가 "fullstack"인 경우, 프론트엔드(React, Vue) 또는 백엔드(Spring, Django) 관련 질문이면 **무조건 is_irrelevant: false**입니다. 문제 설명과의 직접적인 연관성 검사를 하지 마세요.
   - fullstack이 아닌 경우에만: 선택한 프레임워크 및 현재 문제와 무관한 질문인지 판단하세요.
   - 문제 설명과 사용자 질문을 비교하여 판단하세요.
   - 예시:
     * 프레임워크가 "fullstack"인데 "Spring @Autowired가 안 돼요" → is_irrelevant: false (fullstack이므로 무조건 통과)
     * 프레임워크가 "fullstack"인데 "React useEffect 의존성 문제" → is_irrelevant: false (fullstack이므로 무조건 통과)
     * 프레임워크가 "fullstack"인데 "Django ORM 쿼리가 느려요" → is_irrelevant: false (fullstack이므로 무조건 통과)
     * 프레임워크가 "fullstack"인데 "Vue 컴포넌트 props 전달 문제" → is_irrelevant: false (fullstack이므로 무조건 통과)
